...
type TypeInfo = LiteralTypeInfo | PrimitiveTypeInfo | ObjectTypeInfo | UnionTypeInfo | ArrayTypeInfo | UnknownType;
...
function createConfigurationSchemaReference(): string {
    ...
    const entryNodeType = findType('FabricloggerConfigSchema', program, typeChecker);
    ...
    function appendSectionForType(configSchemaType: ts.Type) {
        ...
        const fields: Field[] = [];
        // If this type is extended from another, first append section for its parent,
        // then append all of its parent's fields to this section.
        const baseTypes = configSchemaType.getBaseTypes();
        if (baseTypes && baseTypes?.length > 0) {
            const baseType = baseTypes[0];
            const baseName = baseType.symbol?.name?.replace(/Schema$/, '').replace(/Config$/, '');
            appendSectionForType(baseType);
            const baseSection = sections[baseName];
            fields.push(...baseSection.fields);
        }
    ...
    return Object.values(sections).map(formatSection).join('\n\n\n');
}
...
function generateMetaVariablesReference() {
    ...
    if (members == null) {
        throw new Error('Did not find members for MetadataVariables');
    }
...
async function main() {
    ...
    const exampleContent = await readFile(join(__dirname, '../test/config/example1.fabriclogger.yaml'), {
        encoding: 'utf-8',
    });
