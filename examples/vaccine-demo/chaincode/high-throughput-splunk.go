...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	if function == "item" {
		return s.item(APIstub, args)
	} else if function == "ingredient" {
		return s.ingredient(APIstub, args)
...
func (s *SmartContract) item(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	_ = json.Unmarshal([]byte(ingredients_str), &ingredients)

	object_type := "item"
	item := &item{object_type, item_uid, item_name, item_type, location, date, batch, ingredients, event, min_temp, max_temp}
	itemJSONasBytes, jsonerr := json.Marshal(item)
	if jsonerr != nil {
		return shim.Error(fmt.Sprintf("Error marshaling item to JSON: %s", jsonerr))
	}

	putStateErr := APIstub.PutState(item_uid, itemJSONasBytes)
	if putStateErr != nil {
...
func (s *SmartContract) ingredient(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	ingredient := &ingredient{object_type, ing_uid, ing_name, ing_type, item_uid, location, date, batch, event, min_temp, max_temp}
	itemJSONasBytes, jsonerr := json.Marshal(ingredient)
	if jsonerr != nil {
		return shim.Error(fmt.Sprintf("Error marshaling ingredient to JSON: %s", jsonerr))
	}

	putStateErr := APIstub.PutState(ing_uid, itemJSONasBytes)
	if putStateErr != nil {
